...
class _HomeScreenState extends State<HomeScreen> {
  ...
  // ══════════════════════════════════════════
  // ✅ فتح ProfileScreen مع BlocProvider خاص بيه
  // ══════════════════════════════════════════
  Future<void> _openProfile() async {
    ...
    Navigator.push(
      ...
      MaterialPageRoute(
        builder: (_) => BlocProvider(
          // ✅ بنعمل ProfileCubit جديد خاص بصفحة البروفايل
          create: (_) => getIt<ProfileCubit>(),
          child: ProfileScreen(userId: userId),
        ),
      ),
  ...
  // ══════════════════════════════════════════
  // ✅ Logout — مش محتاجين ProfileCubit
  // بنستخدم FirebaseAuth مباشرة أو AuthCubit لو موجود
  // ══════════════════════════════════════════
  Future<void> _logout() async {
    await FirebaseAuth.instance.signOut();
    if (!mounted) return;
    // ✅ بنروح على صفحة اللوجين
  ...
  // ══════════════════════════════════════════
  Widget _buildTopAppBar() {
    // ✅ بنجيب اسم وإيميل اليوزر من FirebaseAuth مباشرة
    ...
    return Container(
      ...
      child: Row(
        mainAxisAlignment: MainAxisAlignment.spaceBetween,
        children: [
          // ✅ بدون context.read<AuthCubit>() أو context.read<ProfileCubit>()
          ProfileAvatarButton(
            name: displayName,
            email: email,
            onViewProfile: _openProfile,   // ✅ async function منفصلة
            onLogout: _logout,             // ✅ async function منفصلة
          ),
          ...
          RichText(
            text: TextSpan(
              ...
              children: [
                ...
                TextSpan(
                  text: 'Landmarks',
                  style: TextStyle(fontWeight: FontWeight.w600, color: _goldColor),
                ),
              ],
            ),
          ),
                ...
                children: [
                  ...
                  Positioned(
                    ...
                    child: Container(
                      ...
                      child: Text(landmark.category.name.toUpperCase(),
                  ...
                  Positioned(
                    top: 12.h, right: 12.w,
                    child: GestureDetector(
                      onTap: () => setState(() => _favourites[landmark.id] = !isFav),
                      child: Container(
                    ...
                    GestureDetector(
                      ...
                      child: Container(
                        ...
                        decoration: BoxDecoration(
                          ...
                          boxShadow: [BoxShadow(color: AppColors.darkGold.withOpacity(0.45), blurRadius: 14, offset: const Offset(0, 6))],
                        ),
